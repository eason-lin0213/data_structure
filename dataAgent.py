...
async def process_chunk(chunk, start_idx, total_records, model_client, termination_condition):
    ...
    prompt = (
    f"目前正在評估第 {start_idx} 至 {start_idx + len(chunk) - 1} 位球員（共 {total_records} 位）。\n"
    f"以下為該批球員的數據:\n{chunk_data}\n\n"
    "請根據以上數據進行分析，並提供完整的球員選拔建議。"
    "其中請特別注意：\n"
    "  1. 分析球員的比賽數據，包括得分、籃板、助攻、抄截、阻攻與上場時間；\n"
    "  2. 請 MultimodalWebSurfer 搜尋外部網站，找出最新的球員選拔標準與數據分析方法（如 PER、WS、BPM 等），\n"
    "     並將搜尋結果整合進回覆中；\n"
    "  3. 最後請提供具體的選拔建議，包括適合的戰術角色、需要提升的方面，以及可能的最佳位置與球隊配置。\n"
    "請各代理人協同合作，提供一份完整且具參考價值的選拔建議。"
)
...
async def main():
    ...
    csv_file_path = "basketball_stats.csv"
    chunk_size = 1000
    chunks = list(pd.read_csv(csv_file_path, encoding="utf-8", chunksize=chunk_size, encoding_errors="replace"))
